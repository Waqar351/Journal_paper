...
import sys
from applyquantifiers_QT import apply_quantifier_QT
from pandas_to_arff import pandas_arff
import os
...
def Run_expereiment(exp_name, method_name, niterations = 10 ):
   """
   This is the main function to run the experimental setup for Quantification trees methods. It returns performance of QT/QT-ACC in terms MAE across different test sample sizes and training proportions and save the results in .CSV file.
   ...
   niterations : integer
    ...
    ---------
    CSV file
        that contains performance of QT or QT-ACC methods in terms of MAE.
   ...
   tr_prop = [0.05,0.1,0.3,0.5,0.7,0.9]
   tr_sample= 10 
   for train_prop in tr_prop:
       for train_sample in range(tr_sample):            
            ...
            if exp_name == 'handwritten':
                tr["class"].replace({"q": "1", "p": "2","g": "2"}, inplace=True)
                te["class"].replace({"q": "1", "p": "2","g": "2"}, inplace=True)
            
            tr['class'] = tr['class'].astype('str')
            te['class'] = te['class'].astype('str')

            first_col_train = tr.pop('class')
            first_col_test = te.pop('class')
            num_columns = len(tr. columns)
            
            tr.insert(num_columns,'class', first_col_train)
            te.insert(num_columns,'class', first_col_test)
                      
            tr['class'] = tr['class'].astype('str')
            te['class'] = te['class'].astype('str')
            
            #..................Quantificatioin Trees.....................

            tr = tr.drop(["Binary_label"], axis=1)  #dropping class label columns
            tr = pd.DataFrame(tr)
            #pandas2arff(df=tr, filename="./models_train_test/"+exp_name+"/training.arff", wekaname=exp_name)
            pandas_arff(df=tr, filename= folder + '/train_data_%s' % exp_name + '_%f'%train_prop +'.arff' , wekaname=exp_name)
            # the next lines will build the QT model and it will be saved in a file named classifier. We need it to quantify each test set in our experiments.
            command = "java -Xmx6G -cp quantify.jar:weka.jar:. weka.classifiers.trees.RandomForest -I 500 -P 1 -t "
            
            command += folder + '/train_data_%s' % exp_name + '_%f'%train_prop +'.arff'+ " -d " + folder +"/classifier"
            print(command)
            os.system(command)
            ...
            for sample_size in batch_sizes:   #[10,100,500], batch_sizes, Varying test set sizes
                ...
                for alpha in alpha_values: #   Varying positive class distribution
                    ...
                    for iter in range(niterations):
                        ...
                        test_sample = sample_test.drop(["Binary_label"], axis=1)  #dropping class label columns
                        ...
                        pred_pos_prop = apply_quantifier_QT(qntMethod= method_name, test = test_sample ,dt_name = exp_name, folder = folder,**method_kargs)
                        tm_end = timer()
